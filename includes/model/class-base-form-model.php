<?php
/**
 * Meta_Aliexpress_Base_Form_Model Class
 *
 * @since  1.0.0
...
if ( ! class_exists( 'Auto_Aliexpress_Base_Form_Model' ) ) :

	abstract class Auto_Aliexpress_Base_Form_Model {

		const META_KEY = 'auto_aliexpress_form';
		 ...
		 *
		 * @var Auto_Aliexpress_Form_Field_Model[]
		 */
		public $fields = array();
		...
		/**
		 * @since 1.0.0
		 * @return Auto_Aliexpress_Form_Field_Model[]
		 */
		 ...
		 * @param $slug
		 *
		 * @return Auto_Aliexpress_Form_Field|null
		 */
		...
		public function get_all_paged( $current_page = 1, $per_page = null, $status = '' ) {
			if ( is_null( $per_page ) ) {
				$per_page = auto_aliexpress_form_view_per_page();
			}
			$args = array(
				'post_type'      => $this->post_type,
				'post_status'    => 'any',
     ...
     */
						...
						if ( 'post' === $map['type'] ) {
							...
							if ( ! empty( $meta['fields'] ) && 'fields' === $map['field'] ) {
								foreach ( $meta['fields'] as $field_data ) {
									$field          = new Auto_Aliexpress_Form_Field_Model( $form_settings );
		...
		public function get_fields_grouped() {
			...
			foreach ( $this->fields as $field ) {
				/** @var Auto_Aliexpress_Form_Field_Model $field */
