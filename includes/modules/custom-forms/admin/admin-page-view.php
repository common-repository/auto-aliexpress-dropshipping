<?php
/**
 * Auto_Aliexpress_CForm_Page Class
 *
 * @since  1.0.0
...
if ( ! class_exists( 'Auto_Aliexpress_CForm_Page' ) ) :

class Auto_Aliexpress_CForm_Page extends Auto_Aliexpress_Admin_Page {
    ...
    public function processRequest() {
        ...
        if ( ! isset( $_POST['auto_aliexpress_nonce'] ) ) {
            return;
        }

        $nonce = $_POST['auto_aliexpress_nonce'];
        if ( ! wp_verify_nonce( $nonce, 'auto-aliexpress-campaign-request' ) ) {
            return;
        }

        $is_redirect = true;
        $action = "";
        if(isset($_POST['auto_aliexpress_bulk_action'])){
            $action = sanitize_text_field($_POST['auto_aliexpress_bulk_action']);
            $ids = isset( $_POST['ids'] ) ? sanitize_text_field( $_POST['ids'] ) : '';
        }else if(isset($_POST['auto_aliexpress_single_action'])){
            $action = sanitize_text_field($_POST['auto_aliexpress_single_action']);
	...
	public function getModules() {
		$modules = array();
		$limit   = null;
		if ( defined( 'AUTO_ALIEXPRESS_FORMS_LIST_LIMIT' ) && AUTO_ALIEXPRESS_FORMS_LIST_LIMIT ) {
			$limit = AUTO_ALIEXPRESS_FORMS_LIST_LIMIT;
		}
		...
		foreach ( $data['models'] as $model ) {
            ...
            $modules[] = array(
				...
				"status"          => $model->status,
                "type"            => $settings['aliexpress_selected_source'],
                "keywords"        => isset($settings['aliexpress_selected_keywords']) ? $settings['aliexpress_selected_keywords'] : '',
    ...
    public function get_models( $limit = null ) {
        $data = Auto_Aliexpress_Custom_Form_Model::model()->get_all_paged( $this->page_number, $limit );
    ...
    public function bulk_actions() {
        return apply_filters(
            'auto_aliexpress_campaign_bulk_actions',
            array(
                'publish-campaigns'    => esc_html__( "Publish", Auto_Aliexpress::DOMAIN ),
                'draft-campaigns'      => esc_html__( "Unpublish", Auto_Aliexpress::DOMAIN ),
                'delete-campaigns'     => esc_html__( "Delete", Auto_Aliexpress::DOMAIN ),
            ) );
    }
    ...
    public function update_module_status( $id, $status ) {
        ...
        if ( in_array( $status, array( 'publish', 'draft' ), true ) ) {
            $model = Auto_Aliexpress_Custom_Form_Model::model()->load( $id );
            if ( $model instanceof Auto_Aliexpress_Custom_Form_Model ) {
                $model->status = $status;
                $model->save();
            }
        }
    ...
    public function pagination() {
        ...
        auto_aliexpress_list_pagination( $count );
    }
